import webQ.utils.cache.cache_model_view as model  # 这里注意要引入需要生成的model
...
def read_template(dict_string):
    ...
    template = env.get_template('view_query_page.template')
...
def run():
    ...
    # with open('../generated_file/myview_query_page.py', 'w', encoding='utf8') as f:
    #     f.write(read_template(dict_string=dict_string))
    with open(os.path.join(file_path, 'gen_view_query_page.py'), 'w', encoding='utf8') as f:
        f.write(read_template(dict_string=dict_string))
