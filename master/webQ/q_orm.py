...
class Model(dict, metaclass=ModelMetaclass):
    ...
    async def findAll(cls, where=None, args=None, **kw):
        ...
        rs = await select1(' '.join(sql), args)
    ...
    async def find(cls, pk):
        ...
        :return:
        '''
        rs = await select1('%s where `%s`=?' % (cls.__select__, cls.__primary_key__), [pk], 1)
        if len(rs) == 0:
            return None
        return cls(**rs[0])
